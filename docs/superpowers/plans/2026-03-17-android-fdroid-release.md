...
git commit -m "chore(android): set app ID to info.tradeinsight.software.chess3000 and add env-var signing config"
```
...
Create a temporary file `info.tradeinsight.software.chess3000.yml` (anywhere on your machine):
...
cp info.tradeinsight.software.chess3000.yml metadata/
# Then lint using the app ID (no .yml extension, no path prefix)
fdroid lint info.tradeinsight.software.chess3000
```
...
cp /path/to/info.tradeinsight.software.chess3000.yml metadata/
git checkout -b add-chess3000
git add metadata/info.tradeinsight.software.chess3000.yml
git commit -m "New app: Chess3000 (info.tradeinsight.software.chess3000)"
...
Push a new tag (e.g., `v1.0.1`) and verify that F-Droid's build bot creates a new build entry automatically. Check `https://f-droid.org/packages/info.tradeinsight.software.chess3000/` for the new version.
...
| `info.tradeinsight.software.chess3000.yml` | Create locally → submit to fdroiddata (external repo) |
